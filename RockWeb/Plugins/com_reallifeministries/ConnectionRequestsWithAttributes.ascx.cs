...
{
    [DisplayName( "Connection Requests Filtered with Attributes (RLM Custom)" )]
    [Category( "Connection" )]
    [Description( "A list of connection requests, optionally filtered by custom attributes." )]
    ...
    public partial class ConnectionRequestsWithAttributes : Rock.Web.UI.RockBlock
    {
...
";
                ...
                foreach (var attribute in AvailableAttributes)
                {
                    ...
                    if (filterControl != null)
                    {
                        try
                        {
                            ...
                            rFilter.SaveUserPreference( MakeKeyUniqueToOpportunity( attribute.Key ), attribute.Name, attribute.FieldType.Field.GetFilterValues( filterControl, attribute.QualifierValues, Rock.Reporting.FilterMode.SimpleFilter ).ToJson() );
                        }
                        catch
                        {
        ...
        protected void rFilter_DisplayFilterValue( object sender, GridFilter.DisplayFilterValueArgs e )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                if (AvailableAttributes != null)
                {
                    var attribute = AvailableAttributes.FirstOrDefault( a => MakeKeyUniqueToOpportunity(a.Key) == e.Key );
                    if (attribute != null)
                    {
                        try
                        {
                ...
                else if (e.Key == "Location")
                {
                    ...
                    Guid? locationGuid = e.Value.AsGuidOrNull();
        ...
        private void BindAttributes()
        {
            ...
            if (SelectedOpportunityId != null)
            {
                ...
                var opportunity = (new ConnectionOpportunityService( new RockContext() )).Get( SelectedOpportunityId.Value );
                string opportunityQualifier = opportunity.Id.ToString();
                string connectionTypeQualifier = opportunity.ConnectionTypeId.ToString();

                foreach (var attributeModel in new AttributeService( new RockContext() ).Queryable()
                    .Where( a =>
                        a.EntityTypeId == entityTypeId &&
                        a.IsGridColumn &&
                        ((a.EntityTypeQualifierColumn.Equals( "Id", StringComparison.OrdinalIgnoreCase ) && a.EntityTypeQualifierValue.Equals( opportunityQualifier )) ||
                         (a.EntityTypeQualifierColumn.Equals( "ConnectionTypeId", StringComparison.OrdinalIgnoreCase ) && a.EntityTypeQualifierValue.Equals( connectionTypeQualifier ))) )
            ...
            if (AvailableAttributes != null)
            {
                foreach (var attribute in AvailableAttributes)
                {
                    ...
                    if (control != null)
                    {
                        ...
                        string savedValue = rFilter.GetUserPreference( MakeKeyUniqueToOpportunity(attribute.Key) );
                    ...
                    bool columnExists = gRequests.Columns.OfType<AttributeField>().FirstOrDefault( a => a.DataField.Equals( dataFieldExpression ) ) != null;
                    if (!columnExists)
                    {
                            ...
                            foreach (var attribute in AvailableAttributes)
                            {
                                ...
                                if (filterControl != null)
                                {
                                    ...
                                    if (expression != null)
                                    {
                                        ...
                                        requests = requests.Where( w => attributeValues.Select( v => v.EntityId ).Contains( w.Id ) );
                                    }
                                }
                            }
                        }
