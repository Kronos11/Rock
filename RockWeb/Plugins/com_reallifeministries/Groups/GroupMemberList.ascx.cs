...
namespace com.reallifeministries.Groups
{
    [DisplayName( "RLM Group Member List" )]
    [Category( "Groups" )]
    [Description( "Lists all the members of the given group. (Configurable for displaying sub groups)" )]
        ...
        protected void BindGroupMembersGrid( bool selectAll = false )
        {
            if ( _group != null )
            {
                ...
                if ( _group.GroupType.Roles.Any() )
                {
                    ...
                    GroupMemberService groupMemberService = new GroupMemberService( rockContext );
                    var qry = groupMemberService.Queryable( "Person,GroupRole,Group", true ).AsNoTracking()
